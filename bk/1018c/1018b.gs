...
function doPost(e) {
  try {
    ...
    const memberIdFromClient = String(body.memberId || '').trim();
    const scores = body.scores || {};
    const visual = body.visual || null; // ★ 追加 { sakeIndex: number } or null
      ...
      for (let i=0; i<mlen; i++) {
        ...
        if (exhibId && exhibId === senderMemberId) {
          if (val !== undefined && val !== null && val !== '' && !isNaN(val) && Number(val) > 0) {
            return _corsJson({ ok:false, error:`cannot vote your own entry (index ${i})`, version: VERSION });
          }
        }
      }
    }
    ...
    // === ここからビジュアル投票の検証＆保存 ===
    if (visual && typeof visual.sakeIndex === 'number') {
      const vi = visual.sakeIndex | 0; // 整数化
      if (vi < 0 || vi >= mlen) {
        return _corsJson({ ok:false, error:'invalid visual index', version: VERSION });
      }
      // 自己銘柄はNG
      const exhibId = String(master.items[vi].exhibitorMemberId || '').trim();
      if (senderMemberId && exhibId && exhibId === senderMemberId) {
        return _corsJson({ ok:false, error:'cannot visual-vote your own entry', version: VERSION });
      }

      // visual_votes に追記
      const vsh = SpreadsheetApp.getActive().getSheetByName('visual_votes');
      if (vsh) {
        const sakeName = master.items[vi].name || '';
        vsh.appendRow([new Date(), nickname, vi, sakeName]);
      }
    }
    // === ビジュアル投票ここまで ===
